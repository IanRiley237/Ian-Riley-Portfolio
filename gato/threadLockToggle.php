<?php
    ...
    if ($_POST['Is_Locked'])
    {
        $conn->query('  UPDATE Threads
                        SET Is_Locked = 0, Locker_Account_ID = NULL
                        Where Thread_ID = ' . $_POST["Thread_ID"]);
    }
    else
    {
        $conn->query('  UPDATE Threads
                        SET Is_Locked = 1, Locker_Account_ID = ' . $_SESSION["Account_ID"] . '
                        Where Thread_ID = ' . $_POST["Thread_ID"]);
    }
              
    // After changing the value, update the lock and flag icons      
    $isLockedTable = $conn->query('SELECT Thread_ID, Is_Flagged, Is_Locked
                                    From Threads
                                    Where Thread_ID = ' . $_POST["Thread_ID"]);
    $thread = $isLockedTable->fetch_assoc();
?>
